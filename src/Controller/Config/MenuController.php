...
class MenuController extends AbstractController
{
    ...
    public function index(Request $request, MenuRepository $menu, FunctionalityRepository $func, ProfileRepository $pro)
    {
        $params = array_filter(
        ...
        $paginator = $menu->getMenuPaginator($offset, $itemPerPage, $request->query->all());
    ...
    // public function createMenu(Request $request, EntityManagerInterface $em, ProfileRepository $profileRepo, FunctionalityRepository $funcRepo, MenuRepository $menuRepo): Response
    // {
    //     $req = $request->request->all();

    //     $func = !empty($req['functionality']) ? $funcRepo->find($req['functionality']) : null;
    //     $profile = !empty($req['profile']) ? $profileRepo->find($req['profile']) : null;
    //     $menuRep = !empty($req['menu']) ? $menuRepo->find($req['menu']) : null;

    //     $menu = new Menu();

    //     $menu->setIdProfile($profile);
    //     $menu->setIdFunctionality($func);
    //     $menu->setIdMenuParent($menuRep);
    //     $menu->setMenu($req['item']);
    //     $menu->setOrden($req['orden']);
